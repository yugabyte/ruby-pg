...
describe YSQL::TypeMapByColumn do

	let!(:textenc_int){ YSQL::TextEncoder::Integer.new(name: 'INT4', oid: 23).freeze }
	let!(:textdec_int){ YSQL::TextDecoder::Integer.new(name: 'INT4', oid: 23).freeze }
	let!(:textenc_float){ YSQL::TextEncoder::Float.new(name: 'FLOAT4', oid: 700).freeze }
	let!(:textdec_float){ YSQL::TextDecoder::Float.new(name: 'FLOAT4', oid: 700).freeze }
	let!(:textenc_string){ YSQL::TextEncoder::String.new(name: 'TEXT', oid: 25).freeze }
	let!(:textdec_string){ YSQL::TextDecoder::String.new(name: 'TEXT', oid: 25).freeze }
	let!(:textdec_bytea){ YSQL::TextDecoder::Bytea.new(name: 'BYTEA', oid: 17).freeze }
	let!(:binaryenc_bytea){ YSQL::BinaryEncoder::Bytea.new(name: 'BYTEA', oid: 17, format: 1).freeze }
	let!(:binarydec_bytea){ YSQL::BinaryDecoder::Bytea.new(name: 'BYTEA', oid: 17, format: 1).freeze }
	let!(:pass_through_type) do
		type = Class.new(YSQL::SimpleDecoder) do
			def decode(*v)
				v
			end
		end.new
	...
	it "should deny changes when frozen" do
		tm = YSQL::TypeMapByColumn.new([]).freeze
	...
	it "should be shareable for Ractor", :ractor do
		tm = YSQL::TypeMapByColumn.new([pass_through_type]).freeze
	...
	it "should give account about memory usage" do
		...
		tm = YSQL::TypeMapByColumn.new([textenc_float, nil, textenc_int] ).freeze
		expect( ObjectSpace.memsize_of(tm) ).to be > size0
	end

	it "should retrieve it's conversions" do
		cm = YSQL::TypeMapByColumn.new([textdec_int, textenc_string, textdec_float, pass_through_type, nil] ).freeze
	...
	it "should respond to inspect" do
		cm = YSQL::TypeMapByColumn.new([textdec_int, textenc_string, textdec_float, pass_through_type, YSQL::TextEncoder::Float.new, nil] ).freeze
	...
	it "should retrieve it's oids" do
		cm = YSQL::TypeMapByColumn.new([textdec_int, textdec_string, textdec_float, pass_through_type, nil] ).freeze
	...
	it "should gracefully handle not initialized state" do
		...
		not_init = Class.new(YSQL::TypeMapByColumn) do
			def initialize
				# no super call
			end
		end.new.freeze
		...
		@conn.copy_data("COPY (SELECT 1) TO STDOUT") do
			decoder = YSQL::TextDecoder::CopyRow.new(type_map: not_init)
	...
	it "should encode integer params" do
		col_map = YSQL::TypeMapByColumn.new([textenc_int]*3 ).freeze
	...
	it "should encode bytea params" do
		data = "'\u001F\\"
		col_map = YSQL::TypeMapByColumn.new([binaryenc_bytea]*2 ).freeze
		res = @conn.exec_params( "SELECT $1, $2", [ data, nil ], 0, col_map )
		res.type_map = YSQL::TypeMapByColumn.new([textdec_bytea]*2 )
		expect( res.values ).to eq( [
				[ data, nil ],
		] )
	end
	...
	it "should allow hash form parameters for default encoder" do
		col_map = YSQL::TypeMapByColumn.new([nil, nil] ).freeze
	...
	it "should convert hash form parameters to string when using string encoders" do
		col_map = YSQL::TypeMapByColumn.new([textenc_string, textenc_string] ).freeze
		hash_param_bin = { value: ["00ff"].pack("H*"), type: 17, format: 1 }
		hash_param_nil = { value: nil, type: 17, format: 1 }
	...
	it "shouldn't allow param mappings with different number of fields" do
		expect{
			@conn.exec_params("SELECT $1", [ 123 ], 0, YSQL::TypeMapByColumn.new([]).freeze )
		}.to raise_error(ArgumentError, /mapped columns/)
	end

	it "should verify the default type map for query params as well" do
		tm1 = YSQL::TypeMapByColumn.new([]).freeze
		expect{
			@conn.exec_params("SELECT $1", [ 123 ], 0, YSQL::TypeMapByColumn.new([nil]).with_default_type_map(tm1) )
		}.to raise_error(ArgumentError, /mapped columns/)
	end

	it "forwards query param conversions to the #default_type_map" do
		tm1 = YSQL::TypeMapByClass.new
		tm1[Integer] = YSQL::TextEncoder::Integer.new name: 'INT2', oid: 21

		tm2 = YSQL::TypeMapByColumn.new([textenc_int, nil, nil] ).with_default_type_map(tm1 ).freeze
	...
	class Exception_in_decode < YSQL::SimpleDecoder
	...
	it "should raise an error from decode method of type converter" do
		...
		res.type_map = YSQL::TypeMapByColumn.new(types ).freeze
		expect{ res.values }.to raise_error(/no type decoder defined/)
	end

	it "should raise an error for invalid params" do
		expect{ YSQL::TypeMapByColumn.new(:WrongType ) }.to raise_error(TypeError, /wrong argument type/)
		expect{ YSQL::TypeMapByColumn.new([123] ) }.to raise_error(TypeError, /wrong argument type (Integer|Fixnum)/)
	end

	it "shouldn't allow result mappings with different number of fields" do
		res = @conn.exec( "SELECT 1" )
		expect{ res.type_map = YSQL::TypeMapByColumn.new([]) }.to raise_error(ArgumentError, /mapped columns/)
	end
	...
	it "should verify the default type map for result values as well" do
		...
		tm1 = YSQL::TypeMapByColumn.new([]).freeze
		expect{
			res.type_map = YSQL::TypeMapByColumn.new([nil]).with_default_type_map(tm1)
		}.to raise_error(ArgumentError, /mapped columns/)
	end
	...
	it "forwards result value conversions to a TypeMapByOid as #default_type_map" do
		...
		[0, 10].each do |max_rows|
			tm1 = YSQL::TypeMapByOid.new
			tm1.add_coder YSQL::TextDecoder::Integer.new name: 'INT2', oid: 21
			tm1.max_rows_for_online_lookup = max_rows

			tm2 = YSQL::TypeMapByColumn.new([textdec_int, nil, nil] ).with_default_type_map(tm1 ).freeze
	...
	it "get_copy_data returns string with encoding" do
		tm1 = YSQL::TypeMapByColumn.new([textdec_string, textdec_bytea] ).freeze
		decoder = YSQL::TextDecoder::CopyRow.new(type_map: tm1)
	...
	it "forwards get_copy_data conversions to another TypeMapByColumn as #default_type_map" do
		tm1 = YSQL::TypeMapByColumn.new([textdec_int, nil, nil] ).freeze
		tm2 = YSQL::TypeMapByColumn.new([nil, textdec_int, nil] ).with_default_type_map(tm1 ).freeze
		decoder = YSQL::TextDecoder::CopyRow.new(type_map: tm2)
	...
	it "will deny copy queries with different column count" do
		[[2, 2], [2, 3], [3, 2]].each do |cols1, cols2|
			tm1 = YSQL::TypeMapByColumn.new([textdec_int, nil, nil][0, cols1] ).freeze
			tm2 = YSQL::TypeMapByColumn.new([nil, textdec_int, nil][0, cols2] ).with_default_type_map(tm1 ).freeze
			decoder = YSQL::TextDecoder::CopyRow.new(type_map: tm2)
			@conn.copy_data("COPY (SELECT 1, 2, 3) TO STDOUT", decoder) do
	...
	it "should allow mixed type conversions" do
		...
		res.type_map = YSQL::TypeMapByColumn.new([textdec_int, textdec_string, textdec_float, pass_through_type, nil] ).freeze
