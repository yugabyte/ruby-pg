...
describe YSQL::Connection do
	...
	it "tries to connect to localhost with IPv6 and IPv4", :ipv6, :postgresql_10 do
		...
		expect{ described_class.connect( uri ) }.to raise_error(YSQL::ConnectionBad)
	end
	...
	describe "#transaction" do
		...
		it "stops a thread that runs a failing transaction with exec" do
			...
			expect( t.value ).to be_kind_of(YSQL::UndefinedColumn )
			expect( duration ).to be < 10
		end
		...
		it "doesn't worry about an already finished connection" do
			...
			expect( t.value ).to be_kind_of(YSQL::Result )
			expect( t.value.result_status ).to eq(YSQL::PGRES_COMMAND_OK )
		end
	end
	...
	context "OS thread support" do
		...
		it "Connection.new shouldn't block a second thread" do
			...
			t = Thread.new do
				...
				}.to raise_error(YSQL::ConnectionBad, /server closed the connection unexpectedly/)
			end
