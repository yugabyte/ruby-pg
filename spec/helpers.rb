			...
			mod.around( :each ) do |example|
				begin
					...
					if @conn.respond_to?(:exit_pipeline_mode) &&
							@conn.pipeline_status != YSQL::PQ_PIPELINE_OFF
						@conn.pipeline_sync
					...
					@conn.type_map_for_results = YSQL::TypeMapAllStrings.new
					@conn.type_map_for_queries = YSQL::TypeMapAllStrings.new
...
EOT
	...
	class ConnStillUsableMatcher
		...
		def check_for_problems
			...
			return "has bad status" unless @conn.status == YSQL::CONNECTION_OK
			return "has bad transaction status (%d)" % [ @conn.transaction_status ] unless
				@conn.transaction_status.between?(YSQL::PQTRANS_IDLE, YSQL::PQTRANS_INTRANS )
	...
	def wait_for_polling_ok(conn, meth = :connect_poll)
		...
		while status != YSQL::PGRES_POLLING_OK
			if status == YSQL::PGRES_POLLING_READING
				select( [conn.socket_io], [], [], 5.0 ) or
					raise "Asynchronous connection timed out!"

			elsif status == YSQL::PGRES_POLLING_WRITING
				select( [], [conn.socket_io], [], 5.0 ) or
					raise "Asynchronous connection timed out!"

			elsif status == YSQL::PGRES_POLLING_FAILED
				break
			end
			status = conn.send(meth)
		end
	...
	def run_with_scheduler(timeout=10)
		thread_with_timeout(timeout) do
			...
			Fiber.schedule do
				conn = YSQL.connect(@conninfo_gate)
...
RSpec.configure do |config|
	config.include(YSQL::TestingHelpers )
	...
	config.filter_run_excluding( :postgresql_94 ) if YSQL.library_version <  90400
	config.filter_run_excluding( :postgresql_95 ) if YSQL.library_version <  90500
	config.filter_run_excluding( :postgresql_96 ) if YSQL.library_version <  90600
	config.filter_run_excluding( :postgresql_10 ) if YSQL.library_version < 100000
	config.filter_run_excluding( :postgresql_12 ) if YSQL.library_version < 120000
	config.filter_run_excluding( :postgresql_14 ) if YSQL.library_version < 140000
